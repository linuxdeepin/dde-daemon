...
func (cm *CurveManager) getBacklightCurvePercent(c *displayBl.Controller) float64 {
	...
	if cm.curveType == "flm" {
		...
		// findBrightnessPercentByCurve 返回 10-100（百分比），除以100转为0-1
		return findBrightnessPercentByCurve(int32(currentBrightness), cm.flmCurveFunc, cm.backLightMinValue, cm.backlightMidValue, int32(c.MaxBrightness)) / 100.0
	}
	return float64(currentBrightness) / float64(c.MaxBrightness)
}
...
func findBrightnessPercentByCurve(targetBrightness int32, curveFunc func(float64, int) int32, backLightMinValue int32, backlightMidValue int32, maxBrightness int32) float64 {
	const (
		x1            = 10.0
		x2            = 100.0
		maxIterations = 50 // 最大迭代次数，防止无限循环
		tolerance     = 0.01
	)
	...
	if targetBrightness <= midBrightness {
		// 目标亮度在线性段，搜索范围[x1, backlightMidValue]
		left = x1
		right = float64(backlightMidValue)
	} else {
		// 目标亮度在2.2次函数段，搜索范围[backlightMidValue, x2]
	...
	var bestDiff int32 = maxBrightness // 初始化为最大值
