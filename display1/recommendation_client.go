...
// readAmbientState 通过 factory 客户端逐个读取 AmbientBrightness1 属性，
// 返回完整状态。供 RecommendationClient.readState 和 one-shot 查询复用。
func readAmbientState(ambient ambientbrightness1.AmbientBrightness) (RecommendationState, error) {
...
func (c *RecommendationClient) readState() (RecommendationState, error) {
	return readAmbientState(c.ambient)
}
