...
func (m *Manager) claimAmbientLight() {
	...
	err := m.helper.SensorProxy.ClaimLight()
	if err != nil {
		logger.Warning(err)
		return
	}
...
func (m *Manager) releaseAmbientLight() {
	...
	err := m.helper.SensorProxy.ReleaseLight()
	if err != nil {
		logger.Warning(err)
		return
	}
...
func (m *Manager) handleLightLevelChanged() {
	if !m.AmbientLightAdjustBrightness.Get() {
		return
	}

	lightLevel := m.helper.SensorProxy.LightLevel.Get()
	if lightLevel <= 0 {
	...
	outputNames, err := display.ListOutputNames()
	if err != nil {
		logger.Warning(err)
		return
	}
	...
	err = display.SetBrightness(builtinOutputName, br)
	if err != nil {
